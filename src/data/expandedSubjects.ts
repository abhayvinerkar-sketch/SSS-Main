...
export function getExpandedMaterial(subjectId:string,chapterId:string,chapterTitle:string,topics:Topic[]):MaterialSection[]{const c=getChapterContent(subjectId,chapterId,topics);return[{heading:'Key Points',body:c.flatMap(x=>x.keyPoints)},{heading:'Definitions',body:c.map(x=>`${x.title}: ${x.concept}`)},{heading:'Exam Strategy',body:c.map(x=>x.remember||'Revise the concept and write answers in your own words.')}];}
