...
const PLAN_KEY='ssc-master-plan-v1';
...
export async function loadPlanDone():Promise<Record<string,boolean>>{try{const raw=await AsyncStorage.getItem(PLAN_KEY);return raw?JSON.parse(raw):{}}catch{return {}}}
export async function savePlanDone(done:Record<string,boolean>){try{await AsyncStorage.setItem(PLAN_KEY,JSON.stringify(done))}catch{}return done}
